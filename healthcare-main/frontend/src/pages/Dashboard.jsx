...
import WaterIntake from '../components/WaterIntake'
import HealthFeatures from '../components/HealthFeatures'
import MedicineTracker from '../components/MedicineTracker'   // ← NEW
...
function Card({ children, t, style = {}, glowOnHover = false }) {
  return (
    ...
    onMouseEnter={e => {
      e.currentTarget.style.transform = 'perspective(1200px) rotateX(4deg) rotateY(-2deg) translateZ(20px)'
      e.currentTarget.style.boxShadow = t.shadowElevated
      if (glowOnHover) e.currentTarget.style.borderColor = t.primary
    }}
    onMouseLeave={e => {
      e.currentTarget.style.transform = 'perspective(1200px) rotateX(0deg) rotateY(0deg) translateZ(0)'
      e.currentTarget.style.boxShadow = t.shadowCard
      if (glowOnHover) e.currentTarget.style.borderColor = t.border
    }}
    >
      {glowOnHover && (
        <div style={{
          position: 'absolute', inset: 0,
          background: t.primaryGrad, opacity: 0,
          transition: 'opacity 0.3s ease',
...
function SectionHeader({ title, subtitle, action, t }) {
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        <p style={{
          margin: 0, fontSize: '11px', letterSpacing: '0.28em',
          textTransform: 'uppercase', fontWeight: '600', color: t.primary,
          display: 'flex', alignItems: 'center', gap: '8px',
        }}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: t.primary, boxShadow: `0 0 8px ${t.primary}` }} />
          {subtitle}
        </p>
        <h2 style={{
          margin: '10px 0 0', fontSize: '22px', lineHeight: 1.15, fontWeight: '700',
          background: t.primaryGrad, WebkitBackgroundClip: 'text',
...
export default function Dashboard() {
  ...
  const [showUploadMenu, setShowUploadMenu] = useState(false)
  const [showProfileSettings, setShowProfileSettings] = useState(false)
  const [showMedicineTracker, setShowMedicineTracker] = useState(false)   // ← NEW
  const [profileSaving, setProfileSaving] = useState(false)
  const [profileMessage, setProfileMessage] = useState('')
  const [profileError, setProfileError] = useState('')
  const [profileForm, setProfileForm] = useState({
  ...
  // ── ActionRow — 3 buttons now with Medicine Tracker ─────────────────────────
  const ActionRow = () => (
    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
      ...
      <button
        ...
        style={{
          ...
          padding: '20px 10px', borderRadius: '20px', border: 'none',
          background: showNewsSection ? t.primaryGrad : t.surface,
          boxShadow: showNewsSection ? t.shadowElevated : t.shadowCard,
          cursor: 'pointer',
      ...
      >
        <span style={{ fontSize: '28px' }}>📰</span>
        <span style={{ fontSize: '12px', fontWeight: '700', color: showNewsSection ? '#fff' : t.primary }}>Health News</span>
        <span style={{ fontSize: '10px', color: showNewsSection ? 'rgba(255,255,255,0.7)' : t.textMuted }}>Latest</span>
      </button>

      {/* ── NEW: Medicine Tracker ── */}
      <button
        onClick={e => { e.stopPropagation(); setShowMedicineTracker(true); setShowUploadMenu(false); setShowNewsSection(false) }}
        className="btn-3d"
        style={{
          flex: '1 1 100px', minWidth: 0,
          padding: '20px 10px', borderRadius: '20px', border: 'none',
      ...
      >
        ...
        <span style={{ fontSize: '28px' }}>💊</span>
        <span style={{ fontSize: '12px', fontWeight: '800', color: '#a5b4fc' }}>Medicines</span>
        <span style={{ fontSize: '10px', color: 'rgba(165,180,252,0.7)' }}>Track doses</span>
      </button>

      {/* Upload Docs */}
      <div style={{ flex: '1 1 100px', minWidth: 0, position: 'relative' }}>
        <button
          onClick={e => { e.stopPropagation(); setShowUploadMenu(v => !v); setShowNewsSection(false) }}
          className="btn-3d"
          style={{
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          ...
          <button onClick={() => setShowProfileSettings(false)} style={{ border: 'none', background: t.surfaceAlt, cursor: 'pointer', fontSize: '18px', color: t.textMuted, width: '36px', height: '36px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
        </div>

        <form onSubmit={handleSaveProfile} style={{ display: 'grid', gap: '16px' }}>
          {profileError && (
            <div style={{ color: t.error, fontSize: '13px', padding: '12px 16px', background: t.errorBg, borderRadius: '12px', border: `1px solid ${t.errorBorder}` }}>
              {profileError}
            </div>
          )}
          {profileMessage && (
            <div style={{ color: t.success, fontSize: '13px', padding: '12px 16px', background: 'rgba(16,185,129,0.1)', borderRadius: '12px', border: '1px solid rgba(16,185,129,0.3)' }}>
              {profileMessage}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '14px', marginTop: '14px' }}>
            <button type="button" onClick={() => setShowProfileSettings(false)} className="btn-lift" style={modalButtonStyle(t, false)}>Cancel</button>
            <button type="submit" disabled={profileSaving} className="btn-lift" style={modalButtonStyle(t, true)}>
              {profileSaving ? 'Saving...' : 'Save changes'}
            </button>
          </div>
        </form>
    ...
    >
      <div
        ...
        style={{
          ...
          padding: '28px 20px 40px', boxShadow: t.shadowModal,
          border: `1px solid ${t.border}`,
          animation: 'slideUp 0.35s cubic-bezier(0.4, 0, 0.2, 1)',
        }}
      >
        ...
        <label htmlFor="profile-camera-input" className="btn-lift"
          style={{ display: 'flex', alignItems: 'center', gap: '16px', width: '100%', padding: '18px 20px', borderRadius: '18px', background: t.surfaceAlt, cursor: 'pointer', marginBottom: '12px', boxSizing: 'border-box' }}
        >
          <span style={{ width: '50px', height: '50px', borderRadius: '16px', background: t.primaryGrad, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0, boxShadow: `0 4px 12px rgba(79,70,229,0.3)` }}>📷</span>
          <div>
            <div style={{ fontWeight: '700', fontSize: '15px', color: t.text }}>Take Photo</div>
            <div style={{ fontSize: '12px', color: t.textMuted, marginTop: '2px' }}>Open camera directly</div>
          </div>
        </label>
        <input id="profile-camera-input" type="file" accept="image/*" capture="user" style={{ display: 'none' }} onChange={handlePhotoSelected} />

        <label htmlFor="profile-gallery-input" className="btn-lift"
          style={{ display: 'flex', alignItems: 'center', gap: '16px', width: '100%', padding: '18px 20px', borderRadius: '18px', background: t.surfaceAlt, cursor: 'pointer', marginBottom: '12px', boxSizing: 'border-box' }}
        >
          <span style={{ width: '50px', height: '50px', borderRadius: '16px', background: 'linear-gradient(135deg, #f093fb, #f5576c)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0, boxShadow: '0 4px 12px rgba(240,147,251,0.3)' }}>🖼️</span>
          <div>
            <div style={{ fontWeight: '700', fontSize: '15px', color: t.text }}>Choose from Gallery</div>
            <div style={{ fontSize: '12px', color: t.textMuted, marginTop: '2px' }}>Pick from your photos</div>
          </div>
        </label>
        <input id="profile-gallery-input" type="file" accept="image/*" style={{ display: 'none' }} onChange={handlePhotoSelected} />

        {user?.profilePhoto && (
          <button onClick={handleRemovePhoto} className="btn-lift"
            style={{ display: 'flex', alignItems: 'center', gap: '16px', width: '100%', padding: '18px 20px', borderRadius: '18px', border: 'none', background: 'rgba(239,68,68,0.08)', cursor: 'pointer', marginBottom: '8px', boxSizing: 'border-box', fontFamily: 'inherit' }}
          >
            <span style={{ width: '50px', height: '50px', borderRadius: '16px', background: 'linear-gradient(135deg, #f6546a, #e53e3e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0, boxShadow: '0 4px 12px rgba(246,84,106,0.3)' }}>🗑️</span>
            <div>
              <div style={{ fontWeight: '700', fontSize: '15px', color: t.error }}>Remove Photo</div>
              <div style={{ fontSize: '12px', color: t.textMuted, marginTop: '2px' }}>Delete current photo</div>
            </div>
        ...
        <button onClick={() => setShowPhotoOptions(false)} className="btn-lift"
          style={{ width: '100%', marginTop: '16px', padding: '16px', borderRadius: '16px', border: 'none', background: t.surfaceAlt, cursor: 'pointer', fontSize: '15px', fontWeight: '600', color: t.textMuted, fontFamily: 'inherit' }}
        >
          Cancel
        </button>
      </div>
  ...
  const UserCard = ({ compact = false }) => (
    <div style={{
      background: t.primaryGrad, borderRadius: '24px',
      padding: compact ? '20px' : '28px',
      position: 'relative', overflow: 'hidden',
      boxShadow: t.shadowElevated,
      transform: 'perspective(800px) rotateX(1deg)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', position: 'relative' }}>
        <div style={{ position: 'relative' }}>
          <Avatar size={compact ? 56 : 64} />
          <div style={{ position: 'absolute', bottom: 2, right: 2, width: compact ? '14px' : '16px', height: compact ? '14px' : '16px', borderRadius: '50%', background: t.success, border: '3px solid white', boxShadow: `0 2px 8px ${t.success}66` }} />
        </div>
        <div>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '11px', fontWeight: '600', letterSpacing: '0.6px' }}>MY PROFILE</p>
          <p style={{ color: '#fff', fontSize: compact ? '17px' : '19px', fontWeight: '700', letterSpacing: '-0.3px', marginTop: '4px' }}>{user?.name || 'User'}</p>
  ...
  if (isDesktop) {
    return (
      <div onClick={() => { setShowProfileMenu(false); setShowUploadMenu(false) }} style={{ minHeight: '100vh', background: t.pageBg, display: 'flex', flexDirection: 'column' }}>
        <header style={{ background: t.headerBg, backdropFilter: 'blur(20px)', borderBottom: `1px solid ${t.border}`, padding: '0 40px', height: '68px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, boxShadow: t.shadow }}>
          <div onClick={toggleChat} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
            <span style={{ fontSize: '28px' }}>🩺</span>
            <span style={{ fontSize: '20px', fontWeight: '900', background: t.primaryGrad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', letterSpacing: '-0.5px' }}>Health AI</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <button onClick={e => { e.stopPropagation(); toggleTheme() }} style={{ width: '40px', height: '40px', borderRadius: '50%', background: t.surfaceAlt, border: `1px solid ${t.border}`, cursor: 'pointer', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: t.shadow }}>
              {isLight ? '🌙' : '☀️'}
            ...
            <div onClick={e => { e.stopPropagation(); setNotifications(0) }} style={{ position: 'relative', cursor: 'pointer' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: t.surfaceAlt, border: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', boxShadow: t.shadow }}>🔔</div>
              {notifications > 0 && <div style={{ position: 'absolute', top: '-2px', right: '-2px', background: 'linear-gradient(135deg, #f6546a, #e53e3e)', color: '#fff', borderRadius: '50%', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: '700', border: '2px solid white' }}>{notifications}</div>}
            </div>
            <div style={{ position: 'relative' }}>
              <div onClick={e => { e.stopPropagation(); setShowProfileMenu(v => !v) }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 14px 6px 8px', borderRadius: '40px', background: t.surfaceAlt, border: `1px solid ${t.border}`, cursor: 'pointer', boxShadow: t.shadow }}>
                <Avatar size={34} />
        ...
        <div style={{ display: 'flex', flex: 1, gap: 0 }}>
          <aside style={{ width: '260px', flexShrink: 0, background: t.sidebarBg, backdropFilter: 'blur(20px)', borderRight: `1px solid ${t.border}`, padding: '28px 16px', display: 'flex', flexDirection: 'column', gap: '28px', position: 'sticky', top: '68px', height: 'calc(100vh - 68px)', overflowY: 'auto' }}>
            ...
            <WaterIntake />
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {[{ id: 'home', icon: '🏠', label: 'Home' }, { id: 'health', icon: '❤️', label: 'Health Features' }, { id: 'hospitals', icon: '🏥', label: 'Hospitals' }, { id: 'documents', icon: '📁', label: 'My Documents' }].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '11px 14px', borderRadius: '14px', border: 'none', background: activeTab === tab.id ? t.navActive : 'transparent', color: activeTab === tab.id ? t.primary : t.textSub, fontWeight: activeTab === tab.id ? '700' : '500', fontSize: '14px', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.15s ease' }}>
                  <span style={{ fontSize: '18px' }}>{tab.icon}</span>
                  {tab.label}
                  {activeTab === tab.id && <div style={{ marginLeft: 'auto', width: '6px', height: '6px', borderRadius: '50%', background: t.primary }} />}
  ...
  function DropBtn({ children, onClick, t, danger }) {
    return (
      <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', padding: '10px 14px', borderRadius: '12px', border: 'none', background: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: danger ? t.error : t.text, textAlign: 'left', fontFamily: 'inherit' }}>
        {children}
      </button>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.pageBg, display: 'flex', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      <div
        className="container-3d"
        style={{ width: '100%', maxWidth: '520px', minHeight: '100vh', background: t.containerBg, position: 'relative', paddingBottom: '84px', boxShadow: t.shadowXl, transform: 'perspective(1000px)' }}
        onClick={() => { setShowProfileMenu(false); setShowUploadMenu(false) }}
      >
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '280px', background: t.gradientBlob1, filter: 'blur(50px)', opacity: 0.5, pointerEvents: 'none' }} />
        ...
        <div style={{ padding: '16px 18px 14px', background: '#1E3A8A', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${t.glassBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 4px 24px -8px rgba(0,0,0,0.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', position: 'relative' }}>
            ...
            {showProfileMenu && (
              <div style={{ position: 'absolute', top: '54px', left: 0, background: t.surface, borderRadius: '18px', boxShadow: t.shadowMd, padding: '8px', minWidth: '175px', zIndex: 200, border: `1px solid ${t.border}` }}>
                <DropBtn onClick={() => { setShowProfileMenu(false); setShowProfileSettings(true) }} t={t}>✏️  Edit your details</DropBtn>
                <DropBtn onClick={handleChangePhoto} t={t}>🖼️  Change photo</DropBtn>
                {user?.profilePhoto && <DropBtn onClick={handleRemovePhoto} t={t} danger>🗑️  Remove photo</DropBtn>}
                <div style={{ height: '1px', background: t.border, margin: '4px 0' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div onClick={e => { e.stopPropagation(); setNotifications(0) }} style={{ position: 'relative', cursor: 'pointer' }}>
              <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: t.surfaceAlt, border: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>🔔</div>
              {notifications > 0 && <div style={{ position: 'absolute', top: '-2px', right: '-2px', background: 'linear-gradient(135deg, #f6546a, #e53e3e)', color: '#fff', borderRadius: '50%', width: '17px', height: '17px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: '700', border: '2px solid white' }}>{notifications}</div>}
            </div>
          </div>
        </div>
            ...
            <div style={{ paddingTop: '16px' }}>
              <Card t={t}>
                <SectionHeader title="Health updates" subtitle="Insights" t={t}
                  action={
                    <button onClick={e => { e.stopPropagation(); setShowNewsSection(false) }} style={{ minWidth: '48px', height: '36px', borderRadius: '10px', border: `1px solid ${t.border}`, background: t.surfaceAlt, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', color: t.textMuted }}>✕</button>
                  }
                />
                <NewsSection />
              </Card>
        ...
        {/* Bottom nav */}
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%) perspective(1000px) rotateX(5deg)', width: '100%', maxWidth: '520px', background: t.glass, backdropFilter: 'blur(20px) saturate(180%)', WebkitBackdropFilter: 'blur(20px) saturate(180%)', borderTop: `1px solid ${t.glassBorder}`, padding: '12px 8px 24px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 150, boxShadow: '0 -12px 40px -12px rgba(0,0,0,0.18)' }}>
          {[
            { id: 'home', icon: '🏠', label: 'Home' },
            { id: 'health', icon: '❤️', label: 'Health' },
            { id: 'hospitals', icon: '🏥', label: 'Hospitals' },
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="btn-lift" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', background: activeTab === tab.id ? t.navActive : 'transparent', border: 'none', cursor: 'pointer', padding: '10px 18px', borderRadius: '14px', fontFamily: 'inherit', transition: 'all 0.25s cubic-bezier(0.4, 0, 0.2, 1)' }}>
              <span style={{ fontSize: '24px', lineHeight: 1 }}>{tab.icon}</span>
              <span style={{ fontSize: '10px', fontWeight: activeTab === tab.id ? '600' : '500', color: activeTab === tab.id ? t.primary : t.textLight }}>{tab.label}</span>
              {activeTab === tab.id && <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: t.navActiveDot, marginTop: '-3px', boxShadow: `0 0 8px ${t.navActiveDot}` }} />}
            </button>
          ))}
        </div>
      ...
      <style>{`
        ...
        @keyframes fadeIn { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
        @keyframes shimmer { 0%{background-position:-200% 0} 100%{background-position:200% 0} }
        .card-3d { transform: perspective(1200px) rotateX(0deg) rotateY(0deg) translateZ(0); transition: transform 0.4s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
        .card-3d:hover { transform: perspective(1200px) rotateX(4deg) rotateY(-2deg) translateZ(20px); }
        .btn-3d { transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 6px 20px -4px rgba(0,0,0,0.15); }
        .btn-3d:hover { transform: perspective(800px) rotateX(3deg) translateY(-4px) translateZ(10px); box-shadow: 0 12px 30px -6px rgba(0,0,0,0.2); }
        .btn-3d:active { transform: perspective(800px) rotateX(0deg) translateY(0) translateZ(0); transition: all 0.15s ease; }
        .btn-lift { transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }
        .btn-lift:hover { transform: translateY(-2px); }
        .btn-lift:active { transform: translateY(0); }
        .container-3d { transform-style: preserve-3d; }
      `}</style>
    </div>
  )
}
...
function WaterPopup({ t, onYes, onNo }) {
  return (
    <div onClick={onNo} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: '20px', cursor: 'pointer' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: t.surface, borderRadius: '24px', padding: '32px 28px', textAlign: 'center', boxShadow: t.shadowLg, maxWidth: '310px', width: '100%', animation: 'popIn 0.3s cubic-bezier(0.34,1.56,0.64,1)', border: `1px solid ${t.border}`, cursor: 'default' }}>
        <div style={{ fontSize: '52px', marginBottom: '12px' }}>💧</div>
        <h3 style={{ fontSize: '20px', fontWeight: '700', color: t.text, marginBottom: '8px' }}>Stay Hydrated!</h3>
        <p style={{ color: t.textSub, fontSize: '14px', lineHeight: 1.6, marginBottom: '24px' }}>Did you drink water in the last hour?</p>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={onYes} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: 'none', background: t.primaryGrad, color: '#fff', fontWeight: '600', fontSize: '15px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(79,70,229,0.35)', fontFamily: 'inherit' }}>Yes, I did!</button>
          <button onClick={onNo} style={{ flex: 1, padding: '14px', borderRadius: '12px', border: `2px solid ${t.border}`, background: t.surface, color: t.textMuted, fontWeight: '600', fontSize: '15px', cursor: 'pointer', fontFamily: 'inherit' }}>Not yet</button>
        </div>
      </div>
    </div>
  )
